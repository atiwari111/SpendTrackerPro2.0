...
public class CategoryEngine {
    ...
    // Key = lowercase keyword found anywhere in merchant name or SMS
    // Value = category name matching CATEGORIES above
    // ────────────────────────────────────────────────────────────────
    ...
    static {
        ...
        MERCHANT_MAP.put("dunzo",        FOOD); MERCHANT_MAP.put("zepto",       FOOD); // quick commerce food
        ...
        MERCHANT_MAP.put("grofers",      GROC); MERCHANT_MAP.put("blinkit",     GROC);
        MERCHANT_MAP.put("dmart",        GROC); MERCHANT_MAP.put("d mart",      GROC);
        MERCHANT_MAP.put("smart bazar",  GROC); MERCHANT_MAP.put("more",        GROC);
        MERCHANT_MAP.put("jiomart",      GROC); MERCHANT_MAP.put("jio mart",    GROC);
        MERCHANT_MAP.put("spencers",     GROC); MERCHANT_MAP.put("nature basket",GROC);
        MERCHANT_MAP.put("star market",  GROC); MERCHANT_MAP.put("reliance fresh",GROC);
        MERCHANT_MAP.put("spar",         GROC); MERCHANT_MAP.put("hypercity",   GROC);
        MERCHANT_MAP.put("lulu",         GROC); MERCHANT_MAP.put("natures baske",GROC);
        ...
        MERCHANT_MAP.put("floweraura",   GIFT); GIFT.toString();
        MERCHANT_MAP.put("interflora",   GIFT); MERCHANT_MAP.put("archies",      GIFT);
        MERCHANT_MAP.put("hallmark",     GIFT); MERCHANT_MAP.put("gift",         GIFT);
        MERCHANT_MAP.put("flowers",      GIFT); MERCHANT_MAP.put("bouquet",      GIFT);

        // ── PAYMENT GATEWAYS / WALLETS → classify by SMS context not merchant ──
        // These are intermediaries — category resolved via NLP/SMS body fallback
        // (not mapped here to avoid mis-categorising all PhonePe as Bills)
    }
    ...
    // Priority: income keywords > learned > merchant map > NLP > Others
    // ────────────────────────────────────────────────────────────────
    public static String classify(String merchant, String smsBody) {
        ...
        // 1. Income / special transaction detection from SMS body
        ...
        if (body.contains("refund"))
            return "↩️ Refund";
        if (body.contains("interest credit") || body.contains("dividend"))
        ...
        // 4. Also check SMS body for merchant keywords (e.g., UPI note says "swiggy")
        for (Map.Entry<String, String> e : MERCHANT_MAP.entrySet()) {
            if (body.contains(e.getKey())) return e.getValue();
        }

        // 5. NLP token matching fallback
        String nlp = NlpCategorizer.predictWithSource(merchant);
        if (nlp != null) return nlp;
